...
{
    [Authorize]
    public class JosController : Controller
    {
        JosService josService;

        public JosController()
        {
            josService = new JosService();
        }


        public ActionResult Index()
        {
            var menuItem = MenuBuilderService.GetMenuByTitle(ResMenu.Jos);
        ...
        {
            josService.List(ref parametros);
        ...
        {
            var entity = (id == 0 ? new Jos() : josService.GetById(id));
            if (entity == null)
            {
            ...
            ViewBag.Titulo = ResMenu.Jos + " - " + (id == 0 ? ResLabels.Insert : delete ? ResLabels.Delete : locked ? ResLabels.View : ResLabels.Edit);
        ...
        /// <returns></returns>
        [Authorize(Roles = GlobalFunctions.AdmProfile + "," + GlobalFunctions.JsProfile)]
        [HttpPost]
        public JsonResult EditAction(Jos entity, string oldDescription)
        {
            try
            {
                ...
                {
                    var resultado = josService.Save(entity);
        ...
        {
            var data = josService.GetById(id);
            if (data == null)
                ViewBag.Mensagem = ResErrors.RecordNotFound;
            else
            {
        ...
        public JsonResult DeleteAction(int id)
        {
            try
            {
                ...
                var entity = josService.GetById(id);
                if (entity != null)
                {
                    result = josService.Delete(id);
                    if (result.Success)
                    {
        ...
        /// <returns></returns>
        public JsonResult CheckUnique(Jos entity)
        {
            if (string.IsNullOrEmpty(entity.Name))
                return Json(true, JsonRequestBehavior.AllowGet);

            var retorno = "";
            if (josService.GetByFilter(p => p.Id != entity.Id && p.UserName.Equals(entity.UserName, StringComparison.OrdinalIgnoreCase)).Any())
                ...
                using (var context = new DbContext(connectionStringSettings.ConnectionString))
                {
                    using (var transaction = context.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            context.Database.ExecuteSqlCommand("EXEC dbo.InsertEmployees @param", new SqlParameter("@param", 2));
                            LogService.Save(EnumLists.ELogType.Service, "Insert JOS Employees " + result.Message, true, "Manual System", 1);
                            transaction.Commit();
                        }
                        catch (Exception)
                        {
            ...
            try
            {
                ...
                using (var context = new DbContext(connectionStringSettings.ConnectionString))
                {
                    using (var transaction = context.Database.BeginTransaction())
                    {
                        ...
                        {
                            // Executa a procedure com o parâmetro
                            context.Database.ExecuteSqlCommand("EXEC dbo.EmployeesDisableAll @param", new SqlParameter("@param", 2));
                            LogService.Save(EnumLists.ELogType.Service, "Disable JOS Employees " + result.Message, true, "Manual System", 1);
                            transaction.Commit();
                        }
                        catch (Exception)
                        {
            ...
            if (Request.Files.Count > 0)
            {
                ...
                using (StreamReader reader = new StreamReader(Request.Files[0].InputStream))
                {
                    while (!reader.EndOfStream)
                    {
                        ...
                        Jos entity = ConvertLineToWorkday(line);
        ...
        public Jos ConvertLineToWorkday(string line)
        {
            Jos entity = new Jos();
        ...
        public void ChangeNotification(Jos entity)
        {
            if (string.IsNullOrEmpty(entity.EmployeeID))
                return;

            var user = josService.GetByFilter(p => p.EmployeeID.Equals(entity.EmployeeID, StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
            if (user != null)
            {
                user.Notification = true;
                josService.Save(user);
            }
        }
