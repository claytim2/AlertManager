...
namespace Web.Controllers
{
    ...
    public class RhController : Controller
    {
        ...
        {
            var menuItem = MenuBuilderService.GetMenuByTitle(ResMenu.Rh);
        ...
        {
            var entity = (id == 0 ? new Rh() : rhService.GetById(id));
            if (entity == null)
            {
            ...
            ViewBag.Titulo = ResMenu.Rh + " - " + (id == 0 ? ResLabels.Insert : delete ? ResLabels.Delete : locked ? ResLabels.View : ResLabels.Edit);
        ...
        /// <returns></returns>
        [Authorize(Roles = GlobalFunctions.AdmProfile + "," + GlobalFunctions.RhProfile)]
        [HttpPost]
        public JsonResult EditAction(Rh entity, string oldDescription)
        {
            try
            {
                ...
                {
                    var resultado = rhService.Save(entity);
        ...
        public JsonResult DeleteAction(int id)
        {
            try
            {
                ...
                var entity = rhService.GetById(id);
                if (entity != null)
                {
                    result = rhService.Delete(id);
                    if (result.Success)
                    {
        ...
        /// <returns></returns>
        public JsonResult CheckUnique(Rh entity)
        {
            if (string.IsNullOrEmpty(entity.Name))
                return Json(true, JsonRequestBehavior.AllowGet);

            var retorno = "";
            if (rhService.GetByFilter(p => p.Id != entity.Id && p.UserName.Equals(entity.UserName, StringComparison.OrdinalIgnoreCase)).Any())
                ...
                using (var context = new DbContext(connectionStringSettings.ConnectionString))
                {
                    using (var transaction = context.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            context.Database.ExecuteSqlCommand("EXEC dbo.InsertEmployees @param", new SqlParameter("@param", 6));
                            LogService.Save(EnumLists.ELogType.Service, "Insert Rh Employees " + result.Message, true, "Manual System", 1);
                            transaction.Commit();
                        }
                        catch (Exception)
                        {
        ...
        {
            var result = new ResultProcessing { Success = true, Message = "Synchronization successful" };
            try
            {
                var connectionStringSettings = ConfigurationManager.ConnectionStrings["DatabaseContext"];
                if (connectionStringSettings == null || string.IsNullOrWhiteSpace(connectionStringSettings.ConnectionString))
                ...
                using (var context = new DbContext(connectionStringSettings.ConnectionString))
                {
                    using (var transaction = context.Database.BeginTransaction())
                    {
                        ...
                        {
                            // Executa a procedure com o parâmetro
                            context.Database.ExecuteSqlCommand("EXEC dbo.EmployeesDisableAll @param", new SqlParameter("@param", 6));
                            LogService.Save(EnumLists.ELogType.Service, "Disable Rh Employees " + result.Message, true, "Manual System", 1);
                            transaction.Commit();
                        }
                        catch (Exception)
                        {
            ...
            if (Request.Files.Count > 0)
            {
                ...
                using (StreamReader reader = new StreamReader(Request.Files[0].InputStream))
                {
                    while (!reader.EndOfStream)
                    {
                        ...
                        Rh entity = ConvertLineToWorkday(line);
        ...
        public Rh ConvertLineToWorkday(string line)
        {
            Rh entity = new Rh();
        ...
        public void ChangeNotification(Rh entity)
        {
            if (string.IsNullOrEmpty(entity.EmployeeID))
                return;

            var user = rhService.GetByFilter(p => p.EmployeeID.Equals(entity.EmployeeID, StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
            if (user != null)
            {
