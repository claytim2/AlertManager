...
{
    [Authorize]
    public class LpaBelController : Controller
    {
        LpaBelService lpaBelService;

        public LpaBelController()
        {
            lpaBelService = new LpaBelService();
        }

        public ActionResult Index()
        {
            var menuItem = MenuBuilderService.GetMenuByTitle(ResMenu.LpaBel);
        ...
        {
            lpaBelService.List(ref parametros);
        ...
        {
            var entity = (id == 0 ? new LpaBel() : lpaBelService.GetById(id));
            if (entity == null)
            {
            ...
            ViewBag.Titulo = ResMenu.LpaBel + " - " + (id == 0 ? ResLabels.Insert : delete ? ResLabels.Delete : locked ? ResLabels.View : ResLabels.Edit);
        ...
        /// <returns></returns>
        [Authorize(Roles = GlobalFunctions.AdmProfile + "," + GlobalFunctions.LpaBProfile)]
        [HttpPost]
        public JsonResult EditAction(LpaBel entity, string oldDescription)
        {
            try
            {
                ...
                {
                    var resultado = lpaBelService.Save(entity);
        ...
        {
            var data = lpaBelService.GetById(id);
            if (data == null)
                ViewBag.Mensagem = ResErrors.RecordNotFound;
            else
            {
        ...
        public JsonResult DeleteAction(int id)
        {
            try
            {
                ...
                var entity = lpaBelService.GetById(id);
                if (entity != null)
                {
                    result = lpaBelService.Delete(id);
                    if (result.Success)
                    {
        ...
        /// <returns></returns>
        public JsonResult CheckUnique(LpaBel entity)
        {
            if (string.IsNullOrEmpty(entity.Name))
                return Json(true, JsonRequestBehavior.AllowGet);

            var retorno = "";
            if (lpaBelService.GetByFilter(p => p.Id != entity.Id && p.UserName.Equals(entity.UserName, StringComparison.OrdinalIgnoreCase)).Any())
                ...
                using (var context = new DbContext(connectionStringSettings.ConnectionString))
                {
                    using (var transaction = context.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            context.Database.ExecuteSqlCommand("EXEC dbo.InsertEmployees @param", new SqlParameter("@param", 4));
                            LogService.Save(EnumLists.ELogType.Service, "Insert LpaBel Employees " + result.Message, true, "Manual System", 1);
                            transaction.Commit();
                        }
                        catch (Exception)
                        {
            ...
            try
            {
                ...
                using (var context = new DbContext(connectionStringSettings.ConnectionString))
                {
                    using (var transaction = context.Database.BeginTransaction())
                    {
                        ...
                        {
                            // Executa a procedure com o parâmetro
                            context.Database.ExecuteSqlCommand("EXEC dbo.EmployeesDisableAll @param", new SqlParameter("@param", 4));
                            LogService.Save(EnumLists.ELogType.Service, "Disable LpaBel Employees " + result.Message, true, "Manual System", 1);
                            transaction.Commit();
                        }
                        catch (Exception)
                        {
            ...
            if (Request.Files.Count > 0)
            {
                ...
                using (StreamReader reader = new StreamReader(Request.Files[0].InputStream))
                {
                    while (!reader.EndOfStream)
                    {
                        ...
                        LpaBel entity = ConvertLineToWorkday(line);
        ...
        public LpaBel ConvertLineToWorkday(string line)
        {
            LpaBel entity = new LpaBel();
            entity.EmployeeID = line; // Ajuste conforme necessário
                                      // Defina outras propriedades da entidade Workday aqui
            return entity;
        }

        public void ChangeNotification(LpaBel entity)
        {
            if (string.IsNullOrEmpty(entity.EmployeeID))
                return;

            var user = lpaBelService.GetByFilter(p => p.EmployeeID.Equals(entity.EmployeeID, StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
            if (user != null)
            {
                user.Notification = true;
                lpaBelService.Save(user);
            }
        }
