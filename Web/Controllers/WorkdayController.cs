...
namespace Web.Controllers
{
    ...
    public class WorkdayController : Controller
    {
        ...
        {
            var menuItem = MenuBuilderService.GetMenuByTitle(ResMenu.Workday);
        ...
        {
            var employee = new WorkdayService().GetById(id);

            if (employee != null)
            {
                return PartialView("_List", new Workday
                {
        ...
        {
            // workdayService.List(ref parametros);    ALTERADO PRA TENTAR PEGAR FUNCIONARIOS
            new WorkdayService().List(ref parametros);
        ...
        {
            var entity = (id == 0 ? new Workday() : workdayService.GetById(id));
            if (entity == null)
            {
                ViewBag.Titulo = ResErrors.FailedOpenRecord;
                ViewBag.Mensagem = ResErrors.RecordNotFound;
            ...
            ViewBag.Titulo = ResMenu.Workday + " - " + (id == 0 ? ResLabels.Insert : delete ? ResLabels.Delete : locked ? ResLabels.View : ResLabels.Edit);
        ...
        /// <returns></returns>
        [Authorize(Roles = GlobalFunctions.AdmProfile + "," + GlobalFunctions.WdProfile)]
        [HttpPost]
        public JsonResult EditAction(Workday entity, string oldDescription)
        {
            try
            {
                ...
                {
                    var resultado = workdayService.Save(entity);
        ...
        {
            var data = workdayService.GetById(id);
            if (data == null)
                ViewBag.Mensagem = ResErrors.RecordNotFound;
            else
            {
        ...
        public JsonResult DeleteAction(int id)
        {
            try
            {
                ...
                var entity = workdayService.GetById(id);
                if (entity != null)
                {
                    result = workdayService.Delete(id);
                    if (result.Success)
                    {
        ...
        public JsonResult CheckUnique(Workday entity)
        {
            if (string.IsNullOrEmpty(entity.Name))
                return Json(true, JsonRequestBehavior.AllowGet);

            var retorno = "";
            if (workdayService.GetByFilter(p => p.Id != entity.Id && p.UserName.Equals(entity.UserName, StringComparison.OrdinalIgnoreCase)).Any())
                ...
                using (var context = new DbContext(connectionStringSettings.ConnectionString))
                {
                    using (var transaction = context.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            context.Database.ExecuteSqlCommand("EXEC dbo.InsertEmployees @param", new SqlParameter("@param", 1));
                            LogService.Save(EnumLists.ELogType.Service, "Insert Workday Employees " + result.Message, true, "Manual System", 1);
                            transaction.Commit();
                        }
                        catch (Exception)
                        {
            ...
            try
            {
                ...
                using (var context = new DbContext(connectionStringSettings.ConnectionString))
                {
                    using (var transaction = context.Database.BeginTransaction())
                    {
                        ...
                        {
                            // Executa a procedure com o parâmetro
                            context.Database.ExecuteSqlCommand("EXEC dbo.EmployeesDisableAll @param", new SqlParameter("@param", 1));
                            LogService.Save(EnumLists.ELogType.Service, "Disable Workday Employees " + result.Message, true, "Manual System", 1);
                            transaction.Commit();
                        }
                        catch (Exception)
                        {
            ...
            if (Request.Files.Count > 0)
            {
                ...
                using (StreamReader reader = new StreamReader(Request.Files[0].InputStream))
                {
                    while (!reader.EndOfStream)
                    {
                        ...
                        Workday entity = ConvertLineToWorkday(line);
        ...
        public Workday ConvertLineToWorkday(string line)
        {
        ...
        public void ChangeNotification(Workday entity)
        {
            if (string.IsNullOrEmpty(entity.EmployeeID))
                return;

            var workday = workdayService.GetByFilter(p => p.EmployeeID.Equals(entity.EmployeeID, StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
            if (workday != null)
            {
                workday.Notification = true;
