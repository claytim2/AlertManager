...
namespace ServerAlertManager
{
    ...
    public partial class FormLpaVls : Form
    {
        Thread th;
        ...
        private void FormLpaVls_Load(object sender, EventArgs e)
        {
            this.Hide();
            PBNo.Visible = false;
            LblPc.Visible = true;
        ...
        {
            string query = "SELECT COUNT(*) FROM [AlertManager].[dbo].[LpaVls] WHERE EmployeeID = @searchString AND Notification = 1";
        ...
        {
            string query = "SELECT TOP (1) [Message] FROM [AlertManager].[dbo].[Notification] WHERE Area = 'LPAVActions'";
            using (SqlCommand command = new SqlCommand(query, connection))
            {
        ...
        void Done()
        {
            ...
            Process.Start("https://brbelm0apps01/LPAVLS/Actions");
        ...
        private void Msg_install()
        {
            ...
            string Themessage = @"<html>
                                ...
                                <table width=""100%"">
                                <tr>
                                <h1 style=""text-align:center"">LPA VLS Pending Actions</h1>
            ...
            sendHtmlEmail("no-reply@jabil.com", "fernanda_rodrigues@jabil.com", lblMail.Text, Themessage, "LPA NOTIFY", "WARNING", "corimc04", 25);
        }
